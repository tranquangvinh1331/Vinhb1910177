...
class Comic
{
    ...
    {
		$Comics = [];
        $stmt = $this->db->prepare('select * from Comic');
        $stmt->execute();
        while ($row = $stmt->fetch()) {
            $Comic = new Comic($this->db);
            $Comic->fillFromDB($row);
            $Comics[] = $Comic;
        } return $Comics;
...
$result = false;
if ($this->id >= 0) {
$stmt = $this->db->prepare('update Comic set name = :name,
...
'insert into Comic (name, tacgia, notes, created_at, updated_at)
...
{
$stmt = $this->db->prepare('select * from Comic where id = :id');
$stmt->execute(['id' => $id]);
if ($row = $stmt->fetch()) {
$this->fillFromDB($row);
return $this;
...
{
$stmt = $this->db->prepare('delete from Comic where id = :id');
